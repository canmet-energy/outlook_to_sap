    ...
    def create_week_sap_report(self, year_number, week_number):
        ...
        for date in datetime_array:
            ...
            for project_nickname in project_nicknames:
                hours = self.get_hours_spent_on_project_for_date(project_nickname, date)
                print(f"{project_nickname}-{hours}")
                sap_df.loc[ sap_df['project_nickname'] == project_nickname, weekday] = hours
        return sap_df


    def get_hours_spent_on_project_for_date(self, project_nickname, date):
        #filter by date
        date_appointments = self.joined_database[self.joined_database.start.between(date,date + datetime.timedelta(days=1)) ]
        #filter by project
...
pan = OutlookToPandas()
first, last = pan.getDateRangeFromWeek(2019,29)
pan.get_sap_report_for_weeks(2019,[28,29,30])
